...
Summary
-------
This is an evolution of the BiRaitBec WorkBase file. The original [BiRaitBec guide](https://www.nexusmods.com/fallout4/mods/23556) (this version is no longer supported), and the updated guide at the [Collective Modding Discord server](https://discord.gg/DfFYJtt8p4) (which _is_ still being supported) specify that the user install the WorkBase file, but the old one was hard to diagnose and didn't generate clear errors.

This edition of the script attempts to address these shortcomings.

This mod is also available at the [GitHub project](https://github.com/rux616/brb-workbase-improved).
...
Installation
------------
NOTE: The script will automatically attempt to locate and use the original BA2 archives from the Fallout 4 Data folder if it doesn't find it in the `OriginalBa2` folder.
...
Automatic
---------
- Put the repack archives you have downloaded into the `Repack7z` folder.
...
- A dialog box will pop up after the repack archives have been verified asking you to choose an output folder for the patched BA2 archives. By default it goes to the `PatchedBa2` folder, but you can choose a different folder. **Do not choose the Fallout 4 Data folder.**
- The script will then proceed to extract the the repack archives and create the patched BA2 archives.

Notes:

- If using the Main repack with the Performance repack, the texture noted in the guide as being corrupt, `Textures\interiors\vault\VltHallResPaneled07Cafeteria02_Damage_d.DDS`, is preserved from the Performance repack
- If using the Main repack without the Performance repack, the texture noted in the guide as being corrupt, `Textures\interiors\vault\VltHallResPaneled07Cafeteria02_Damage_d.DDS`, is deleted
- If using the Restyle repack, the "Author's Choice" options are automatically chosen
...
- Screaminglake and CyberDanz: For the [Community Modding discord server and the updated BRB guide](https://discord.gg/DfFYJtt8p4)
...
Contact
=======
If you find a bug or have a question about the mod, please post it on the [mod page at Nexus Mods](), or in the [GitHub project](https://github.com/rux616/brb-workbase-improved).
