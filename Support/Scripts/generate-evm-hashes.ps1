...
foreach ($ba2File in $ba2Files) {
    ...
    "`$var = `"originalBa2FileHashes`"; `$keyType = `"FileName`"; `$tags = `"Fallout 4 v1.10.984.0-14349213`"" | Out-File -LiteralPath $logFile -Append
    ...
    $relativeFileList | ForEach-Object {
        ...
        "Add-Hash -VariableName `$var -KeyType `$keyType -Hash `"$hash`" -Tags `$tags -FileName `"$_`" -FileSize $size" | Out-File -LiteralPath $logFile -Append
    }
    "Remove-Variable `"var`", `"keyType`", `"tags`"" | Out-File -LiteralPath $logFile -Append
